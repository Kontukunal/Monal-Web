import React, { useState } from "react";
import { useParams, Link, Navigate } from "react-router-dom";
import Header from "../components/Header";
import Footer from "../components/Footer";
import { Eyebrow, Pill, ArrowGlyph } from "../components/Decor";
import { services } from "../data/constants";
import { useUiAnimations } from "../hooks/useUiAnimations";

/* Stable display order — drives the prev / next service navigation. */
const ORDER = ["pre", "production", "post"];

/* ------------------------------------------------------------------ *
 *  A single expandable discipline row. Uses the CSS grid-rows
 *  0fr â†’ 1fr trick for a smooth, height-agnostic open/close.
 * ------------------------------------------------------------------ */
const DisciplineRow = ({ index, item, isOpen, onToggle }) => (
  <div
    data-reveal="up"
    className={`group border-b border-line transition-colors duration-300 ${
      isOpen ? "bg-mist/60" : "hover:bg-mist/40"
    }`}
  >
    <button
      type="button"
      onClick={onToggle}
      aria-expanded={isOpen}
      className="w-full flex items-center gap-5 md:gap-8 text-left py-6 md:py-8 px-4 md:px-7"
    >
      <span
        className={`font-display text-xl md:text-2xl tabular-nums shrink-0 transition-colors duration-300 ${
          isOpen ? "text-accent" : "text-muted group-hover:text-ink"
        }`}
      >
        {String(index + 1).padStart(2, "0")}
      </span>

      <h3
        className={`flex-1 font-display capitalize tracking-tight leading-none text-2xl md:text-[2.4rem] transition-colors duration-300 ${
          isOpen ? "text-ink" : "text-ink/80 group-hover:text-ink"
        }`}
      >
        {item.name}
      </h3>

      <span
        className={`shrink-0 grid place-items-center w-11 h-11 md:w-12 md:h-12 rounded-full border transition-all duration-300 ${
          isOpen
            ? "bg-accent border-accent text-ink rotate-45"
            : "border-line text-ink group-hover:border-ink"
        }`}
        aria-hidden="true"
      >
        <svg
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2.5"
          strokeLinecap="round"
          className="w-4 h-4 md:w-5 md:h-5"
        >
          <path d="M12 5v14M5 12h14" />
        </svg>
      </span>
    </button>

    {/* Animated reveal */}
    <div
      className={`grid transition-[grid-template-rows] duration-500 ease-[cubic-bezier(.65,0,.2,1)] ${
        isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
      }`}
    >
      <div className="overflow-hidden">
        <p className="font-script-desc text-muted text-base md:text-lg leading-relaxed pb-8 pl-[3.4rem] md:pl-[5.3rem] pr-4 md:pr-20 max-w-3xl">
          {item.desc}
        </p>
      </div>
    </div>
  </div>
);

const ServiceDetail = () => {
  const { slug } = useParams();
  useUiAnimations();

  /* Resolve the service entry from its URL slug. */
  const key = ORDER.find((k) => services[k].slug === slug);
  const [open, setOpen] = useState(0);

  /* Unknown slug â†’ bounce back to the landing page. */
  if (!key) return <Navigate to="/" replace />;

  const service = services[key];
  const orderIndex = ORDER.indexOf(key);
  const prev = ORDER[(orderIndex - 1 + ORDER.length) % ORDER.length];
  const next = ORDER[(orderIndex + 1) % ORDER.length];

  return (
    <>
      <Header />

      {/* ---------------------------------------------------------- *
       *  Hero — centered title banner over the background image
       * ---------------------------------------------------------- */}
      <section className="relative bg-ink text-paper overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={service.img}
            alt={service.title}
            className="w-full h-full object-cover opacity-30"
          />
          <div className="absolute inset-0 bg-ink/55" />
          <div className="absolute inset-0 bg-linear-to-t from-ink via-ink/35 to-ink/55" />
        </div>

        {/* Back link */}
        <div className="absolute top-28 md:top-32 left-6 md:left-12 z-10">
          <Link
            to="/#services"
            className="group inline-flex items-center gap-2.5 text-[12px] font-semibold uppercase tracking-[0.18em] text-white/60 hover:text-white transition-colors"
          >
            <ArrowGlyph className="w-3.5 h-3.5 rotate-180 transition-transform group-hover:-translate-x-1" />
            All capabilities
          </Link>
        </div>

        <div className="relative max-w-[1500px] mx-auto px-6 md:px-12 pt-40 md:pt-48 pb-16 md:pb-24 text-center">
          <div data-reveal="up" className="mb-6 flex justify-center">
            <Eyebrow tone="light">
              {service.number} — {service.eyebrow}
            </Eyebrow>
          </div>

          <h1
            data-split
            className="headline-vibrant font-display capitalize text-[clamp(2.4rem,8vw,6.5rem)] leading-[0.9] tracking-[-0.04em] mx-auto"
          >
            {service.title}
          </h1>
        </div>
      </section>

      {/* ---------------------------------------------------------- *
       *  Disciplines — centered interactive accordion
       * ---------------------------------------------------------- */}
      <section className="relative bg-paper py-20 md:py-28">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <p
            data-reveal="up"
            className="font-script-desc text-center text-muted text-lg md:text-xl leading-relaxed mb-12 md:mb-16"
          >
            {service.intro}
          </p>

          <div className="border-t border-line">
            {service.detail.map((item, i) => (
              <DisciplineRow
                key={item.name}
                index={i}
                item={item}
                isOpen={open === i}
                onToggle={() => setOpen(open === i ? -1 : i)}
              />
            ))}
          </div>

          <div data-reveal="up" className="mt-14 flex justify-center">
            <Pill as={Link} to="/#contact" variant="dark">
              Discuss a project
            </Pill>
          </div>
        </div>
      </section>

      {/* ---------------------------------------------------------- *
       *  Prev / Next service navigation
       * ---------------------------------------------------------- */}
      <section className="relative bg-ink text-paper">
        <div className="grid sm:grid-cols-2">
          {/* Previous */}
          <Link
            to={`/services/${services[prev].slug}`}
            className="group relative overflow-hidden flex flex-col justify-center gap-4 px-6 md:px-12 py-12 md:py-16 border-b sm:border-b-0 sm:border-r border-white/10"
          >
            <img
              src={services[prev].img}
              alt=""
              aria-hidden="true"
              className="absolute inset-0 w-full h-full object-cover scale-110 opacity-0 group-hover:opacity-20 transition-opacity duration-700"
            />
            <div className="absolute inset-0 bg-linear-to-r from-ink via-ink/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

            <span className="relative flex items-center gap-2.5 text-[11px] font-semibold uppercase tracking-[0.22em] text-white/45 group-hover:text-accent transition-colors">
              <ArrowGlyph className="w-4 h-4 rotate-180 transition-transform duration-300 group-hover:-translate-x-1.5" />
              Previous
            </span>
            <span className="relative flex items-baseline gap-4">
              <span className="font-display text-xl md:text-2xl tabular-nums text-white/25">
                {services[prev].number}
              </span>
              <span className="font-display capitalize text-3xl md:text-[2.6rem] leading-none tracking-tight group-hover:text-accent transition-colors">
                {services[prev].eyebrow}
              </span>
            </span>
          </Link>

          {/* Next */}
          <Link
            to={`/services/${services[next].slug}`}
            className="group relative overflow-hidden flex flex-col justify-center items-end text-right gap-4 px-6 md:px-12 py-12 md:py-16"
          >
            <img
              src={services[next].img}
              alt=""
              aria-hidden="true"
              className="absolute inset-0 w-full h-full object-cover scale-110 opacity-0 group-hover:opacity-20 transition-opacity duration-700"
            />
            <div className="absolute inset-0 bg-linear-to-l from-ink via-ink/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

            <span className="relative flex items-center gap-2.5 text-[11px] font-semibold uppercase tracking-[0.22em] text-white/45 group-hover:text-accent transition-colors">
              Next
              <ArrowGlyph className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1.5" />
            </span>
            <span className="relative flex items-baseline gap-4">
              <span className="font-display capitalize text-3xl md:text-[2.6rem] leading-none tracking-tight group-hover:text-accent transition-colors">
                {services[next].eyebrow}
              </span>
              <span className="font-display text-xl md:text-2xl tabular-nums text-white/25">
                {services[next].number}
              </span>
            </span>
          </Link>
        </div>
      </section>

      <Footer />
    </>
  );
};

export default ServiceDetail;
